<h3 class="gdlr-lms-admin-head" ><?php _e('My Talks', 'gdlr-lms'); ?></h3>
...
<?php 
	foreach( $manual_check_results as $result ){ 
		...
		echo '<div class="gdlr-quiz-section-text">' . (empty($result->section_quiz)? __('Final quiz', 'gdlr-lms'):  __('Talk quiz : section', 'gdlr-lms') . ' ' . $result->section_quiz) . '</div>';
		echo '</div>';
		echo '</td>';
