<h3 class="gdlr-lms-admin-head" ><?php _e('Attended Talks', 'gdlr-lms'); ?></h3>
<table class="gdlr-lms-table">
<tr>
	<th><?php _e('Talk Name', 'gdlr-lms'); ?></th>
...
<?php 
	...
	$attendance = get_user_meta($current_user->data->ID, 'gdlr-lms-attendance', true); 
	$attendance = empty($attendance)? array(): $attendance;
	foreach($results as $result){
		if(empty($attendance[$result->course_id]) || $attendance[$result->course_id] == 'yes'){ continue; }
