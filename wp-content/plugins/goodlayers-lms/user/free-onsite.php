<h3 class="gdlr-lms-admin-head" ><?php _e('Free Onsite Talks', 'gdlr-lms'); ?></h3>
...
<?php 
	...
	foreach($results as $result){
		...
		if( strtotime($course_options['start-date']) <= strtotime(date('Y-m-d')) ){
			_e('attended', 'gdlr-hotel');
		}else if( $result->payment_status == 'reserved' ){
			_e('reserved', 'gdlr-hotel');
		}else{
		...
		echo __('Cancel Talk', 'gdlr-lms') . '</a>';
		echo '</td>';
		echo '</tr>';
	}
?>
